...
async def handle_help_command(message):
    ...
    help_embed.add_field(
        ...
        value=f"`{COMMANDS['image']} <prompt>` - Generate an image on the grid.\n"
              f"Example: `{COMMANDS['image']} a neon cyberpunk power grid at night`",
        inline=False
    )
...
async def handle_image_command(message):
    """Handle the !image command — generate image(s) on the grid and post them.

    Open to everyone in active channels (it's the whole point — replace the
    standalone image bot). Keeps it lean: parse the prompt, show typing, submit
    to the grid, upload the result as a Discord attachment."""
    import io

    prompt = message.content[len(COMMANDS['image']):].strip()
    if not prompt:
        await message.reply(
            f"Give me something to draw! Usage: `{COMMANDS['image']} a neon power grid at night`"
        )
        return
    if len(prompt) > 1000:
        prompt = prompt[:1000]
    ...
    try:
        async with message.channel.typing():
            result = await grid_client.generate_image(prompt)
    except Exception as e:
        await message.reply(f"⚠️ Image generation failed: {e}")
        return
    ...
    if not images:
        await message.reply("⚠️ No image came back from the grid.")
        return

    # Download each hosted image and attach it (more reliable than posting URLs,
    # which can expire / not embed). Fall back to the URL if a download fails.
    files, fallback_urls = [], []
    for i, url in enumerate(images):
        try:
            img_resp = requests.get(url, timeout=30)
            img_resp.raise_for_status()
            files.append(discord.File(io.BytesIO(img_resp.content), filename=f"aigarth_{i+1}.webp"))
        except Exception as e:
            print(f"image download failed ({url}): {e}")
            fallback_urls.append(url)

    model = result.get("model", "grid")
    caption = f"🎨 **{prompt[:180]}**\n_model: {model} · powered by AI Power Grid_"
    if files:
        await message.reply(content=caption, files=files)
    if fallback_urls:
        await message.reply("Some images couldn't be attached, here are the links:\n" + "\n".join(fallback_urls))
...
async def on_message(message):
    ...
    if message.channel.id in BOT_CHANNELS:
        ...
        if message.content.startswith(COMMANDS['image']):
